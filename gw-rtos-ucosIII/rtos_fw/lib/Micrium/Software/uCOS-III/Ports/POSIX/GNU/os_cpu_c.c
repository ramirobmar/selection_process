...
/*                                                                                           
static  CPU_TMR_INTERRUPT  OSTickTmrInterrupt = { CPU_TMR_INTERRUPT.Interrupt.NamePtr  = "Tick tmr interrupt",
                                                  .Interrupt.Prio     =  10u,
...
void  OSTaskCreateHook (OS_TCB  *p_tcb)
{
    ...
    p_tcb_ext =(OS_TCB_EXT_POSIX*) malloc(sizeof(OS_TCB_EXT_POSIX));
    p_tcb->ExtPtr = p_tcb_ext;

    ERR_CHK(sem_init(&p_tcb_ext->InitSem, 0u, 0u));
    ERR_CHK(sem_init(&p_tcb_ext->Sem, 0u, 0u));

    OSThreadCreate(&p_tcb_ext->Thread, (void *)OSTaskPosix, (void *)p_tcb, THREAD_CREATE_PRIO);
...
void  OS_CPU_SysTickInit (void)
{
	...
	OSTickTmrInterrupt.Interrupt.NamePtr  = (CPU_CHAR *)"Tick tmr interrupt";
	OSTickTmrInterrupt.Interrupt.Prio     = (CPU_INT08U)10u;                            
	OSTickTmrInterrupt.Interrupt.TraceEn  = (CPU_BOOLEAN)DEF_DISABLED;
	OSTickTmrInterrupt.Interrupt.ISR_Fnct = (void (*)(void))OSTimeTickHandler;              
	OSTickTmrInterrupt.Interrupt.En       = (CPU_BOOLEAN)DEF_ENABLED;
	OSTickTmrInterrupt.OneShot            = (CPU_BOOLEAN)DEF_NO;                         
	OSTickTmrInterrupt.PeriodSec          = (CPU_INT32U) 0u;
	OSTickTmrInterrupt.PeriodMuSec        = (CPU_INT32U)(1000000u / OS_CFG_TICK_RATE_HZ);
                            ...
                            int           prio)
{
    ...
    ERR_CHK(pthread_create(p_thread, &attr,(void* (*)(void*)) p_task, p_arg));
}
